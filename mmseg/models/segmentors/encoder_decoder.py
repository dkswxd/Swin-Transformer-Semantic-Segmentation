        ...
        if len(self.test_cfg.stride) == 2:
            ...
            if rescale:
                preds = resize(
                    ...
                    align_corners=self.align_corners,
                    warning=False)
        elif len(self.test_cfg.stride) == 3:
            h_stride, w_stride, z_stride = self.test_cfg.stride
            h_crop, w_crop ,z_crop = self.test_cfg.crop_size
            batch_size, _, h_img, w_img, z_img = img.size()
            num_classes = self.num_classes
            h_grids = max(h_img - h_crop + h_stride - 1, 0) // h_stride + 1
            w_grids = max(w_img - w_crop + w_stride - 1, 0) // w_stride + 1
            ...
            for h_idx in range(h_grids):
                for w_idx in range(w_grids):
                    for z_idx in range(z_grids):
                        ...
                        x1 = max(x2 - w_crop, 0)
                        z1 = max(z2 - z_crop, 0)
                        crop_img = img[:, :, y1:y2, x1:x2, z1:z2]
                        crop_seg_logit = self.encode_decode(crop_img, img_meta)
                        preds += F.pad(crop_seg_logit,
