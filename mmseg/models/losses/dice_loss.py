...
class GDL(nn.Module):
    def __init__(self, apply_nonlin=None, batch_dice=False, do_bg=True, smooth=1.,
                 square=False, square_volumes=False, loss_weight=1.0):
        """
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        volumes = y_onehot.sum(axes) + 1e-6 # add some eps to prevent div by zero
...
class DC_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs=dict(), ce_kwargs=dict(ignore_index=255,reduction='none'),
                 aggregate="sum", square_dice=False, weight_ce=1, weight_dice=1,
                 log_dice=False, loss_weight=1.0, use_sigmoid=None):
        """
    ...
    def forward(self, net_output, target,weight,ignore_index):
        ...
        if ignore_index is not None:
            target_ = target.clone().unsqueeze(1)
            mask = target_ != ignore_index
            target_[~mask] = 0
            mask = mask.float()
        else:
            target_ = target
        ...
        dc_loss = self.dc(net_output, target_, loss_mask=mask) if self.weight_dice != 0 else 0
        if self.log_dice:
